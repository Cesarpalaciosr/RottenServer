...
export const GetCommentsNumber = async (
  ...
  const result = await comment.find({ idMovie: req.body.idMovie });
...
export const DeleteComment = async (
  ...
  const nota = await comment.findOne({ _id: req.body.idMovie });
  if (!nota) {
    return res.status(400).json({ msg: "El Comentario que busco no existe" });
  }

  const notas = await comment.deleteOne({ _id: req.body.idMovie });
  console.log(notas);
